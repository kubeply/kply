...
/// Stable identifier for a Kubeply session.
...
/// Lifecycle status for a Kubeply session.
...
pub fn render_human_plan(plan: &SessionPlan) -> String {
    ...
    output.push_str("Kubeply session plan\n");
...
mod tests {
    ...
    fn human_plan_mentions_route_header() {
        let plan = SessionPlan::new(
            ...
            Some(RouteHeader::new("x-kubeply-session", "fix-123")),
        ...
        assert!(rendered.contains("route_header: x-kubeply-session=fix-123"));
    }
}
